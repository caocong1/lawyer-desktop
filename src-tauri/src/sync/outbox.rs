...
pub async fn enqueue_feedback(pool: &Pool<Sqlite>, payload_json: &str) -> anyhow::Result<FeedbackOutboxRow> {
    let id = Uuid::new_v4().to_string();
    let now = Utc::now().to_rfc3339();
    sqlx::query(
        "INSERT INTO feedback_outbox (id, payload_json, status, attempt_count, next_retry_at, created_at, updated_at) \
         VALUES (?, ?, 'pending', 0, ?, ?, ?)",
    )
    .bind(&id)
    .bind(payload_json)
