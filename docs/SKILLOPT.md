...
| **Rubric 评委** | LLM 对照评测 rubric 打分 | 结构化质量维度 |
...
3. 切到 **「设置」** 标签：
   ...
   - **评测数据根**：确认包含可用于回放的评测材料目录；
4. 点 **保存设置**。

### 5.3 管理员侧：跑基准测试

1. **「测试用例」** 标签 → 从反馈沉淀或手动准备评测用例；
2. 对目标用例点 **「运行」**；
...
| 4 | — | 管理员 `Ctrl+Shift+O` → 跑验证用例 → 启动精炼 → 审阅采纳 |
...
| `skillopt_eval_data_roots` | 路径列表 | 评测专用材料目录 |
...
vendor/ai-for-china-legal/learning-materials/<case>/evaluation/gold-rubric.md
```
...
**Q：为什么要有验证用例？**
A：闸门需要 **可重复运行的真实任务 + rubric**，用于判断 skill 改动是否真的改善输出。
